<template>
  <v-app>
    ...
    <v-row>
      <v-col cols="12" sm="6">
        ...
        <v-card class="product-display-card">
          ...
          <v-row>
            <v-col v-for="(products, index) in prod" :key="id" cols="12" sm="4">
              <v-card @click="showQuantity(index)">
                <v-card-title>{{ products.name }}</v-card-title>
                <v-card-subtitle>{{ products.category }}</v-card-subtitle>
                <v-card-subtitle>{{ products.price }}</v-card-subtitle>
                <v-card-subtitle>{{ products.quantity }}</v-card-subtitle>
                <td><v-btn @click="removeProduct(products.name)">Remove Product</v-btn></td>
              </v-card>
            </v-col>
          </v-row>
    ...
    <v-row>
      <v-col cols="12">
        <v-card class="order-management-card">
          <v-card-title>Order Management</v-card-title>
          <v-card-text>
            <v-btn @click="assignToDelivery">Assign to Delivery</v-btn>
            <v-btn @click="completeOrder">Complete Order</v-btn>
          </v-card-text>
        </v-card>
      </v-col>
    </v-row>
...
export default {
  ...
  computed: {
    totalProducts() { return this.prod.length; }, averagePrice() { if (this.prod.length === 0) return 0; const total = this.prod.reduce((sum, product) => sum + product.price, 0); return total / this.prod.length; },
  },
  methods: {
    addProductDialog() {
...
removeProduct(productId) {
    ...
    assignToDelivery() {
      if (this.tableData.length > 0) {
        axios.post('/api/assign-order', { products: this.tableData })
          .then(response => {
    ...
    completeOrder() {
      if (this.tableData.length > 0) {
        axios.post('/api/complete-order', { products: this.tableData })
          .then(response => {
    ...
    fetchProducts() {
      ...
      axios.get('/api/products') // Replace with your actual endpoint
        .then(response => {
